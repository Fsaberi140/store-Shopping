...
function UserPanelPage() {
  ...
  useEffect(() => {
    ProductService.getProductUser() // request zu Server
      ...
      .then(({ data }) => {
        // wenn es die Daten(Produkte) bekommt,
  ...
  const deleteProduct = () => {
    ProductService.deleteProduct(id) //request zu Server
      .then(({ data }) => {
        ...
        setProducts((products) => products.filter((item) => item.id !== id)); // die wird von Browser auch gleichzeitig gelöscht
      })
      .catch((error) => {
            ...
            products.map((item, index) => (
              <tr key={item.id}>
                ...
                <td className="text-center">
                  ...
                  <Link
                    to={`/user-panel/create-edit-form/${item.id}`}
                    title="edit"
