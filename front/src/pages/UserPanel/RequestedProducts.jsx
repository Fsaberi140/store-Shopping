          ...
          {data.map((item) => (
            <tr>
              ...
              <td className="text-center">
                <Link
                  to={`/user-panel/requestedProducts/${item.id}`}
                  title="View"
                  className={`border border-none border-warning bg-white rounded-3 bi ${
                    item.status === "Pending" ? "bi-pencil-fill" : "bi-eye-fill"
                  } edit-icon text-warning px-3`}
                />
              </td>
            </tr>
          ))}
